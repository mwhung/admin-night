    ...
    return (
        <div className={cn("space-y-4", className)}>
            <div className="flex justify-between items-end gap-3">
                <div>
                    {showHeading && (
                        <h3 className="text-sm font-medium uppercase tracking-widest text-muted-foreground">Weekly Marker</h3>
                    )}
                    <p className={cn("font-light text-foreground/90", showHeading ? "text-2xl" : "text-[1.65rem]")}>
                        {current.toLocaleString()} <span className="text-base text-muted-foreground/50">/ {target.toLocaleString()} steps recorded</span>
                    </p>
                </div>
            ...
            <p className="type-card-support text-right italic">
                {100 - progress < 1 ? 'Weekly marker reached.' : `${Math.ceil(target - current)} steps left for this week's marker.`}
            </p>
        </div>
    )
}
